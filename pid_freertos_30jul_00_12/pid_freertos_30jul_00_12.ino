...
void setup()
{
        ...
        xTaskCreatePinnedToCore(task_PID_temperature, "PID temp", 20*1024, NULL, 3, &pidTempTaskHandle,0);
...
void task_PID_temperature(void* pvParameters){
  
        PID pid_temperature(&pidTempArgs.input,
                            &pidTempArgs.output,
                            ...
                            DIRECT);
        //turn the PID on
        pid_temperature.SetMode(AUTOMATIC);
        
        while(1){


                // Si el error es menor al umbral, disminuir el máximo de la salida
                if(pidTempArgs.setpoint-pidTempArgs.input<pidTempArgs.threshold){
                        pid_temperature.SetOutputLimits(0, pidTempArgs.windowSize*pidTempArgs.output_limit_min);
                }else{
                        pid_temperature.SetOutputLimits(0, pidTempArgs.windowSize*pidTempArgs.output_limit_max);
                }


                // Actualiza el valor de salida del PID
                pid_temperature.Compute();
                pidToActuator(&pidTempArgs);
...
void task_MONITOR(void* pvParameters){
        ...
        while(1){
                pidTempArgs.input = dht.readTemperature();
                pidHumArgs.input = dht.readHumidity();
